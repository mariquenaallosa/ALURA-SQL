...
SELECT 
  CONVERT(CODIGO_DEL_PRODUCTO USING utf8) AS CODIGO,
  CONVERT(NOMBRE_DEL_PRODUCTO USING utf8) AS DESCRIPCION,
  SABOR,
  TAMANO,
  ENVASE,
  PRECIO_DE_LISTA AS PRECIO_LISTA
FROM jugos_ventas.tabla_de_productos
WHERE CONVERT(CODIGO_DEL_PRODUCTO USING utf8) NOT IN (SELECT CODIGO FROM tb_producto);



SHOW CREATE TABLE tb_producto;
SHOW CREATE TABLE jugos_ventas.tabla_de_productos;
